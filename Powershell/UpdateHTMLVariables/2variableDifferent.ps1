...
$dr1 = "4.03"
(Get-Content $spath) -Replace '(?<=var increment\s=\s).*', $dr -Replace '(?<=var STEPS_PER_ROTATION\s=\s).*', "1" |Set-Content $opath$name"1".html
...
(Get-Content $spath) -Replace '(?<=var increment\s=\s).*', $dr -Replace '(?<=var STEPS_PER_ROTATION\s=\s).*', "2" |Set-Content $opath$name"2".html
